...
public abstract class AgentChecker<T> extends AlarmChecker<T> {
    ...
    public List<String> getWebHookMessages() {
        ...
        for (Entry<String, T> detected : detectedAgents.entrySet()) {
            messages.add(String.format(" Value of agent(%s) is %s%s during the past 5 mins.(Threshold : %s%s)", detected.getKey(), detected.getValue(), unit, rule.getThreshold(), unit));
        }
        return messages;
    }
    ...
    public CheckerValue getCheckerValue() {
        return new AgentCheckerValue(unit, getAgentValues());
    }
